...
require(['zepto', 'lib', "log"], function ($, lib, log, bindPhone) {
    ...
    var login={
        ...
        checkkey:function(){
            var that=this;
            $('.f1').on("input propertychange",function(e){
                if($(this).hasClass('f1')){that.clic("f1");}
        ...
        locf1:function(){
            ...
            $(".btn").click(function(){
                var tx=$(this).data("bg");
                var phoneVal=$(".f1 input:eq(0)").val()
                if($(this).data("ng") && tx=="f1"){
                    ...
                    lib.ajx(lib.getReq().ser + "/member/bindMobile.action", {
                        ...
                        if(data.infocode=='0'){
                            location = common.tools.getUrlParam("backUrl") ? common.tools.getUrlParam("backUrl") : "/login/ziliao.html" + location.search;
                            this_.storage.bindPhone=phoneVal.substr(0,3)+"****"+phoneVal.substr(8,4);
