...
require(['zepto', 'lib','visitor-logs'], function($, lib, vl){
    ...
    $(function(){
        ...
        $(document).on("touchstart", ".ui-getRPB-100", function(){
            var pocketId = lib.getUrlParam('rptId')?lib.getUrlParam('rptId'):170;
        ...
        $(document).on("touchstart", ".ui-getRPB-300", function(){
            var pocketId = lib.getUrlParam('rptId')?lib.getUrlParam('rptId'):172;
            lib.ajx(lib.getReq().ser + '/redPacket/takeRedPacket.action', {redPacketTypeId: pocketId}, function(data){
                if(data.infocode === "0"){
                    alert("您已抢到300元红包");
                }else{
        ...
        $(document).on("touchstart", ".ui-getRPB-500", function(){
            var pocketId = lib.getUrlParam('rptId')?lib.getUrlParam('rptId'):173;
            lib.ajx(lib.getReq().ser + '/redPacket/takeRedPacket.action', {redPacketTypeId: pocketId}, function(data){
                if(data.infocode === "0"){
                    alert("您已抢到500元红包");
                }else{
