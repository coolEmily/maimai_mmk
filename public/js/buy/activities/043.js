...
require(['zepto', 'lib','visitor-logs'], function($, lib, vl){
    ...
    $(function(){
        ...
        $(document).on("touchstart", ".ui-getRPB-50", function(){
            var pocketId = lib.getUrlParam('rptId')?lib.getUrlParam('rptId'):106;
        ...
        $(document).on("touchstart", ".ui-getRPB-100", function(){
            var pocketId = lib.getUrlParam('rptId')?lib.getUrlParam('rptId'):107;
            lib.ajx(lib.getReq().ser + '/redPacket/takeRedPacket.action', {redPacketTypeId: pocketId}, function(data){
                if(data.infocode === "0"){
                    alert("您已抢到100元红包");
                }else{
        ...
        $(document).on("touchstart", ".ui-getRPB-200", function(){
            var pocketId = lib.getUrlParam('rptId')?lib.getUrlParam('rptId'):108;
            lib.ajx(lib.getReq().ser + '/redPacket/takeRedPacket.action', {redPacketTypeId: pocketId}, function(data){
                if(data.infocode === "0"){
                    alert("您已抢到200元红包");
                }else{
