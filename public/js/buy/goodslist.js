...
require(['zepto', 'lib', 'swiper'], function($, lib){
    ...
    $(function(){
        ...
        window.data = {
            ...
            'page': 1,
            'rows': 20,
            'bigMemberId': common.tools.getUrlParam('sId') || $.cookie('maimaicn_s_id') || sessionStorage.bigMemberId,
            'locationType': common.tools.getUrlParam('lt') || sessionStorage.locationType,
            'memberMainPageLocationId': common.tools.getUrlParam('memberMainPageLocationId') || sessionStorage.memberMainPageLocationId,
            };
        if(common.tools.getUrlParam('lt') && common.tools.getUrlParam('sId') && common.tools.getUrlParam('memberMainPageLocationId')){
          sessionStorage.bigMemberId = common.tools.getUrlParam('mId') || $.cookie('maimaicn_s_id');
          sessionStorage.locationType = common.tools.getUrlParam('lt');
          sessionStorage.memberMainPageLocationId = common.tools.getUrlParam('memberMainPageLocationId');
        }
        $.extend(liebiao, {
            slider1: "",
            ...
            sName: 'couponEsc',
            sList: ['couponEsc','price', 'sellShowNumber', 'collectNum'],
            gKeys: lib.getUrlParam("gk") ? lib.getUrlParam("gk") : common.tools.getUrlParam('goodsKeywords'),
            rows: window.data.rows,
            flag: lib.getUrlParam("flag") || sessionStorage.flag || 1,
            imgH: (document.documentElement.clientWidth * 0.8 * 0.495) + "px",
            maxImgH: (640* 0.8 * 0.495) + "px",
            memberId: common.tools.getUrlParam('mId'),
            url: "/mainPageSource/getRecommendList.action",
            init: function(){
                var _t = this;
                sessionStorage.flag = _t.flag;
                if(!history.state){
                    //初始化history
                    history.pushState({'page': window.data.page, 'scrollTop': 0}, '', "?id=" + _t.id + "&couponEsc=0&gk=" + _t.gKeys);
                }else if(!_t.isflash){
                    window.data.rows = history.state.page * _t.rows;
                }else{
                    history.replaceState({'page': window.data.page, 'scrollTop': 0}, '', "?id=" + _t.id + "&couponEsc=0&gk=" + _t.gKeys);
                }
            ...
            tabClick: function(){
                        ...
                        common.js.ajx(reqUrl.ser+"goodsClass/getMemberGoodsClassInfo.action", data, function(data){
                            if(data.infocode === "0"){
                                ...
                                $.each(data.info.list_goodsClass, function(k, v) {
                                    h += '<a href="/buyer/liebiao.html?id='+ v.classId + '&mId=' + _t.memberId +'">'+ v.className +'</a>';
                                });
                                h += '</div>';
                ...
                $('.mall_tab a').click(function(){
                    noMoreGoods = false;
                    var data = {
                        'classId': window.data.classId,
                        'page': 1,
                        'rows': window.data.rows,
                        'bigMemberId': sessionStorage.bigMemberId,
                        'locationType': sessionStorage.locationType,
                        'memberMainPageLocationId': sessionStorage.memberMainPageLocationId,
                    };
                    ...
                    history.replaceState({'page': 1, 'scrollTop': 0}, '', "?id=" + _t.id + "&"　+ (_t.sName + "=" + (window.data[_t.sName] ? window.data[_t.sName] : 0) + "&gk=" + _t.gKeys));
            ...
            getGoodsList: function(data, flag){
                ...
                common.js.ajx(reqUrl.ser + _t.url, data, function(data){
                    if(data.infocode === '0' && !flag){
                        ...
                        if(flag) $("#ui-goods-list").empty();
                        var html = "";
                        if(data.info.picList.length > 0){
                          $(".ui-header-img").html("<a href='+ data.info.picList[0].linkUrl +'><img src='"+ reqUrl.imgPath + data.info.picList[0].imgAUrl +"' style='width:100%; margin-bottom: 10px'></a>");
                        }
                        $.each(gList, function(v){
                            var sImg = '';
                            if(gList[v].activeSignImg && gList[v].activeSignImg !== "8"){
                               var sImg ='<img src="/images/buy/list_' + gList[v].activeSignImg + '.png" />';
                            }

                            var url = '/g?sId=' + $.cookie('maimaicn_s_id') +"&gId="+gList[v].goodsId;
